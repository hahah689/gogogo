# ecommerce_support_agent/app/services/tools/woocommerce.py

import httpx
from langchain_core.tools import tool
from ecommerce_support_agent.app.core.settings import get_settings
...
def search_products(query: str, limit: int = 10) -> List[Dict]:
    ...
    """
    logger.info(f"馃攳 WooCommerce search_products called with query: '{query}', limit: {limit}")
    ...
    with httpx.Client(verify=False, timeout=30.0) as client:  # Disable SSL verification for local dev
        try:
            ...
            logger.info(f"馃搳 Response status: {response.status_code}")
            response.raise_for_status()
            products = response.json()
            
            logger.info(f"鉁?Successfully retrieved {len(products)} products")
...
def search_orders(search_type: str, search_value: str, limit: int = 10) -> List[Dict]:
    ...
    """
    logger.info(f"馃攳 WooCommerce search_orders called with search_type: '{search_type}', search_value: '{search_value}', limit: {limit}")
    ...
    logger.info(f"馃實 Final API URL for orders: {url}")
    ...
    logger.info(f"馃摝 Request params for orders: {params}")
    logger.info(f"馃敆 Request URL: {url}")
    ...
    with httpx.Client(verify=False, timeout=60.0) as client:  # Increased timeout for orders
        try:
            logger.info(f"馃殌 Making API request to orders endpoint: {url}")
            response = client.get(
                url,
                params=params,
                auth=httpx.BasicAuth(settings.WOOCOMMERCE_CONSUMER_KEY, settings.WOOCOMMERCE_CONSUMER_SECRET)
            ...
            logger.info(f"馃搳 Order search response status: {response.status_code}")
            response.raise_for_status()
            ...
            logger.info(f"鉁?Successfully retrieved {len(orders)} orders")
            
            # Log search results for debugging
            if len(orders) == 0:
                logger.warning(f"鈿狅笍 No orders found for {search_type} search with value '{search_value}'.")

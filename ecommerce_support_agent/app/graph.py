...
from ecommerce_support_agent.app.core.humanloop_manager import humanloop_adapter

from ecommerce_support_agent.app.core.state import State
from ecommerce_support_agent.app.core.logger import logger
from ecommerce_support_agent.app.services.utils import (
  create_tool_node_with_fallback,
  create_entry_node,
)
# Import guardrail agents
from ecommerce_support_agent.app.services.guardrails.guardrail_agents import (
...
from ecommerce_support_agent.app.services.assistants.assistant_base import (
  Assistant,
  CompleteOrEscalate,
  llm,
)
...
from ecommerce_support_agent.app.services.tools.woocommerce import (
  search_products,
  search_orders,
)
from ecommerce_support_agent.app.services.tools.forms import submit_form
from ecommerce_support_agent.app.services.tools.blog import search_blog_posts
from ecommerce_support_agent.app.services.assistants.form_submission_assistant import (
  form_submission_assistant,
  ToFormSubmission,
)
from ecommerce_support_agent.app.services.assistants.blog_search_assistant import (
  blog_search_assistant,
  ToBlogSearch,
)

from ecommerce_support_agent.app.services.assistants.primary_assistant import (
  primary_assistant,
...
from ecommerce_support_agent.app.services.assistants.flight_booking_assistant import (
  flight_booking_assistant,
  update_flight_safe_tools,
  update_flight_sensitive_tools,
)
from ecommerce_support_agent.app.services.assistants.car_rental_assistant import (
  car_rental_assistant,
  book_car_rental_safe_tools,
  book_car_rental_sensitive_tools,
)
from ecommerce_support_agent.app.services.assistants.hotel_booking_assistant import (
  hotel_booking_assistant,
  book_hotel_safe_tools,
  book_hotel_sensitive_tools,
)
from ecommerce_support_agent.app.services.assistants.excursion_assistant import (
  excursion_assistant,
...
def user_info(state: State, config: RunnableConfig):
    user_info_str = (
        "当前用户是中国跨境电商卖家。"
        "用户可能咨询的问题包括：商品申诉、平台误判、儿童产品证书误判、"
        "商品页面合规检查、平台沟通邮件、客户售后回复、英文申诉信生成、"
        "产品资料整理、成人非玩具说明、目标年龄说明、页面风险词修改等。"
        "系统应优先围绕跨境电商业务处理，不要查询或讨论航班、酒店、租车、旅游推荐等原项目内容。"
    )
...
def guardrail_check(state: State, config: RunnableConfig):
    """Temporarily bypass guardrail for DeepSeek compatibility testing."""
    logger.info("Guardrail temporarily bypassed for DeepSeek compatibility test.")
    return {"messages": []}
...
def route_woocommerce_tools(state: State) -> Literal["woocommerce", "primary_assistant"]:
    logger.info(f"馃 WooCommerce tools routing check")
    if should_route_to_primary(state):
        logger.info(f"鉃★笍 Routing from WooCommerce tools back to primary assistant")
        return "primary_assistant"
    else:
        logger.info(f"鉃★笍 Routing from WooCommerce tools back to WooCommerce assistant")
        return "woocommerce"
